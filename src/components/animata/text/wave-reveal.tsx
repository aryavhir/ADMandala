...
const createDelay = ({ offset, index, delay }: Pick<ReducedValue, "offset" | "delay"> & { index: number }) => {
...
const createAnimatedNodes = (args: ReducedValue, word: string, index: number): ReducedValue => {
  ...
  const className = cn("inline-block opacity-0 transition-all ease-in-out fill-mode-forwards", {
    // Determine the animation direction
    "animate-reveal-down": !isUp && !blur,
    "animate-reveal-up": isUp && !blur,
    "animate-reveal-down animate-content-blur": !isUp && blur,
    "animate-reveal-up animate-content-blur": isUp && blur,
  }, args.className);
...
export default function WaveReveal({
  ...
  return (
    <div
      className={cn(
        "relative flex flex-wrap justify-center px-2 text-4xl font-black md:px-6 md:text-7xl",
        className,
      )}
    >
      {nodes}
